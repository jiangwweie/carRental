...
3. **架构分层**:
   - `domain/` 层禁止导入 I/O 框架（aiohttp、fastapi、yaml）
...
## 技术约束
1. **数据类型**:
   - 金额/价格计算必须使用 `decimal.Decimal`
   - 禁止使用 `float` 进行金额计算

2. **异步规范**:
   - 可用性检查必须异步执行（`async def check_availability`）
   - 禁止阻塞事件循环

3. **架构分层**:
   - 业务逻辑放在 `src/domain/booking_service.py`
   - 数据库操作放在 `src/infrastructure/booking_repository.py`

## 验收标准
- [ ] 实现完整预约流程
- [ ] 单元测试覆盖率 ≥ 80%
- [ ] 通过 `pytest tests/unit/test_booking_service.py -v`
- [ ] 通过 `mypy --strict src/domain/booking_service.py`

## 边界条件
- **空值处理**: 用户ID或车辆ID为 None 时如何处理
- **并发安全**: 同一车辆同一时段多用户同时预约时是否竞态
- **异常处理**: 数据库异常时是否回滚

## 输出要求
- 修改：`src/domain/booking_service.py`
- 测试：`tests/unit/test_booking_service.py`
- 文档：`docs/features/booking.md`（如需要）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
