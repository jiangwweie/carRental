...
> **技能类型**: 代码审查 + 自动修复
> **适用场景**: 所有涉及金额计算的代码提交
> **审查红线**: float 污染、Decimal 精度、价格格式化
...
### 红线 3: 价格计算前的 Decimal 格式化

**检测脚本** (`scripts/check_decimal_format.py`):
...
def check_decimal_format_in_file(filepath: Path) -> list[dict]:
    """检查价格计算是否有 Decimal 格式化"""
    errors = []
    source = filepath.read_text()
    tree = ast.parse(source)
            ...
            if isinstance(node.func, ast.Attribute):
                if node.func.attr in PRICE_CALC_CALLS:
                    ...
                    if parent and isinstance(parent, ast.Return):
                        if not has_decimal_or_quantize(parent.value):
                            errors.append({
                                "file": str(filepath),
                                "line": node.lineno,
                                "call": node.func.attr,
                                "issue": f"Return value may need Decimal formatting",
                                "suggestion": "Ensure value is Decimal type"
                            })

    return errors

def has_decimal_or_quantize(node: ast.AST) -> bool:
    ...
    for child in ast.walk(node):
        ...
        if isinstance(child, ast.Attribute):
            if child.attr in ['total_price', 'daily_rate', 'deposit_amount']:
                return True
    return False
  ...
  type-check:
    ...
    steps:
      ...
      - name: Check Decimal quantize
        run: |
          python3 scripts/check_decimal_format.py
          if [ $? -ne 0 ]; then
            echo "❌ decimal format check failed"
            exit 1
          fi
...
| 位置 | 问题 | 建议修复 |
|------|------|----------|
| src/domain/booking_service.py:45 | float type annotation | Use Decimal instead |
| src/domain/booking_service.py:52 | float() call | Use Decimal() |

### 价格格式化失败 (阻塞)

| 位置 | 调用 | 问题 |
|------|------|------|
| src/infrastructure/payment_gateway.py:123 | calculate_total() | 返回值缺少 Decimal 格式化 |

### 修复建议

1. 将 `price: float` 改为 `price: Decimal`
2. 在返回价格前添加 `.quantize(Decimal("0.01"))`
```
