...
**检查项**：
- [ ] `domain/` 层未导入 `requests`, `fastapi`, `yaml`
...
# ✅ 正确：使用具名 Pydantic 类
def process(data: OrderInput) -> OrderResult:
    ...
```
...
- `backend/app/domain/rental_engine.py`
...
# 识别简化机会
Agent(subagent_type="code-simplifier", prompt="分析 backend/app/domain/rental_engine.py 的复杂度，识别可简化区域")

# 复杂 Bug 分析
Agent(subagent_type="systematic-debugging", prompt="审查发现的并发问题：多个通知同时触发导致重复推送")
```
