...
### 领域层红线
`domain/` 目录**严禁**导入：requests, fastapi, yaml
...
class CompactCarTrigger(BaseModel):
    type: Literal["compact"]
    params: CompactParams

class SUVTrigger(BaseModel):
    type: Literal["suv"]
    params: SUVParams

Trigger = Annotated[
    Union[CompactCarTrigger, SUVTrigger],
...
async def test_strategy_runner():
    ...
    # Act
    result = await runner.evaluate(order)
...
logger.info("正常业务流程")
logger.warning(f"[ORDER_REJECTED] order_id=... filter=... reason=...")
logger.error("操作失败")
```
...
```python
# 实现完成后调用简化
Agent(subagent_type="code-simplifier", prompt="请简化 backend/app/domain/rental_engine.py 的代码")

# 遇到 Bug 时调用调试
Agent(subagent_type="systematic-debugging", prompt="测试失败：test_rental_engine.py::test_and_node - 分析原因")
```
